...
export default function ModelRegistry() {
    ...
    const [form, setForm] = useState({ name: "", version: "", framework: "", description: "", project: "", experiment_id: "", dataset_version: "", artifact_url: "", approved_by: "", size_mb: "", tags: "" });
    ...
    const handleCreate = async (e) => {
        ...
        const created = await apiClient.entities.Model.create(payload);
        setModels(prev => [created, ...prev]);
        setShowForm(false);
        setForm({ name: "", version: "", framework: "", description: "", project: "", experiment_id: "", dataset_version: "", artifact_url: "", approved_by: "", size_mb: "", tags: "" });
    };
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-[#1f1f23]">
                            {["Name", "Version", "Framework", "Stage", "Project", "Dataset", "Size", "Registered", "Actions"].map(h => (
                    ...
                    <tbody className="divide-y divide-[#1a1a1d]">
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={9} className="text-center text-zinc-500 py-16">No models found</td>
                            </tr>
                        ) : (
                            filtered.map(model => (
                                <tr key={model.id} className="hover:bg-white/[0.025] transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-zinc-500 text-xs max-w-[140px] truncate">{model.dataset_version || "—"}</td>
                        ...
                        <form onSubmit={handleCreate} className="p-6 space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[
                                    ...
                                    { key: "experiment_id", label: "Experiment ID", placeholder: "exp-abc123" },
                                    { key: "dataset_version", label: "Dataset Version", placeholder: "imagenet@3.2.0" },
                                    { key: "artifact_url", label: "Artifact URL", placeholder: "s3://ml-artifacts/model/v1" },
                                    { key: "approved_by", label: "Approved By", placeholder: "alice@acme.ai" },
                                ].map(({ key, label, required, placeholder }) => (
                                    <div key={key} className={key === "artifact_url" ? "sm:col-span-2" : ""}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs text-zinc-400 mb-1.5">Size (MB)</label>
                                    <Input
                                        type="number" min={0}
                                        value={form.size_mb}
                                        onChange={e => setForm(f => ({ ...f, size_mb: e.target.value }))}
                                        placeholder="512"
                                        className="bg-[#18181b] border-[#27272a] text-white text-sm placeholder:text-zinc-600 focus:border-indigo-500"
                                    />
                                </div>
                                <div>
                                    ...
                                    <Input
                                        ...
                                        placeholder="nlp, production, bert"
                                        className="bg-[#18181b] border-[#27272a] text-white text-sm placeholder:text-zinc-600 focus:border-indigo-500"
                                    />
                                </div>
                            </div>
